...
NAME="fcmp_message_sender"

CMD="fcmp_message_sender"
...
start() {
    check_running
    echo "starting $CMD ..."
    #"$BASEDIR"/bin/"$CMD" -d "$EXECUTEDIR" 2>"$EXECUTEDIR"/runtime/logs/"$NAME".err >"$EXECUTEDIR"/logs/"$NAME".out &
    "$BASEDIR"/bin/"$CMD" -d "$EXECUTEDIR" 2>"$EXECUTEDIR"/runtime/"$NAME".err > /dev/null &
    PID=$!
    echo $PID > "$PID_FILE"
	sleep 1
	status
